...
int runAsDaemon(map<int, string> confMap)
{

  // create a forkValue for use in event handling
  pid_t forkValue;
  ...
  myfile << "Running in daemon mode" << endl;
          ...
          if(event->len)
            {
              if(event->mask & IN_MODIFY)
                {
		  // Spawn a child process to handle a modification event in 
		  // the watched directory
		  forkValue = fork();

		  // Handle event if you're the child then exit
		  if(forkValue == 0)
		    {
		      // Logs modification
		    ...
		    {
		      myfile << "Error in forking and responding to modification event. Exiting..." << endl;
		      return 1;
		    }

		  // Continue this program if you're the parent
		}
            }
